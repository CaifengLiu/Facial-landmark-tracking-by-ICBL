function [Data] = load_all_data_flip_path( root_path,imlist, options )
...
%         % flipping the image
%         flip_iimg = floor((iimgs+1)/2);
%     else
%         flip_iimg = iimgs;
%     end
    %% load images
    img = im2uint8(imread([root_path slash imlist{iimgs}]));
    fname = [root_path slash imlist{iimgs}(1:end-3) 'pts'];
