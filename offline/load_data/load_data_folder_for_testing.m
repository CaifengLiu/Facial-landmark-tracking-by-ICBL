function [Data] = load_data_folder_for_testing( dbpath_img, dbpath_pts, options )
...
imlist = dir([dbpath_img slash 'image*.*g']);
nimgs  = length(imlist);
%nimgs  = 10;
...
for iimgs = 1 : nimgs
    ...
    if options.num_pts ==68
        fname = [dbpath_pts slash imlist{iimgs}(1:end-3) 'pts'];
        Data{iimgs}.shape_gt = double(pts68_annotation_load( fname ));
    elseif options.num_pts ==29
        fname = [dbpath_pts slash imlist(iimgs).name(1:end-4) '_original.ljson'];
        data = loadjson(fname);
        tmp = cell2mat(data.groups{1}.landmarks);
        for i = 1:options.num_pts
            shape(i,:) = tmp(i).point;
        end
        shape = fliplr(shape);
        Data{iimgs}.shape_gt = double(shape);
    else 
    ...
    %% normalized the image to the mean-size
